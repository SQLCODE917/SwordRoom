...
export function CommandStatusPanel({ status }: CommandStatusPanelProps) {
  ...
  return (
    <div className={className} role="status" aria-live="polite">
      ...
      <div className="c-cmd__msg t-small">{status.message}</div>
      <div className="c-cmd__msg t-small">errorCode: {status.errorCode ?? ' '}</div>
      <div className="c-cmd__msg t-small">errorMessage: {status.errorMessage ?? ' '}</div>
